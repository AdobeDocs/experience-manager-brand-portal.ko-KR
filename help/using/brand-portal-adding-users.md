...
[[!UICONTROL Admin Console]](https://adminconsole.adobe.com/enterprise/overview)에서 조직과 관련된 모든 제품을 볼 수 있습니다. 제품은 Adobe Analytics, Adobe Target 또는 Experience Manager Assets Brand Portal과 같은 모든 Experience Cloud 솔루션이 될 수 있습니다. AEM Brand Portal 제품을 선택하고 제품 프로필을 만들어야 합니다.
...
>[!NOTE]
>
>직접 또는 Brand Portal에서 [Adobe [!UICONTROL Admin Console]](https://adminconsole.adobe.com/enterprise/overview)에 로그인할 수 있습니다. 직접 로그인하는 경우 아래 절차의 4-7단계에 따라 사용자를 추가합니다.
...
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 새로 추가된 사용자에게 시작 이메일이 전송됩니다. 초대된 사용자는 시작 이메일의 링크를 클릭하여 Brand Portal에 액세스할 수 있습니다. Admin Console에 구성된 전자 메일 ID([!UICONTROL Adobe ID], [!UICONTROL Enterprise ID] 또는 [!UICONTROL 가져온 ID])를 사용하여 로그인할 수 있습니다. 자세한 내용은 [처음 로그인 경험](../using/brand-portal-onboarding.md)을 참조하세요.

   >[!NOTE]
   >
   >사용자가 Brand Portal에 로그온할 수 없는 경우 조직 관리자는 Adobe [!UICONTROL Admin Console]을(를) 방문하여 사용자가 존재하며 하나 이상의 제품 프로필에 추가되었는지 확인해야 합니다.
...
1. [!UICONTROL 새 프로필 만들기] 페이지에서 프로필 이름, 표시 이름, 프로필 설명을 제공하고 프로필에서 사용자를 추가하거나 제거할 때 사용자에게 이메일로 알릴 것인지 선택하십시오.

   ![제품 프로필 만들기](assets/admin_console_addaproductprofilecreatenewprofile.png)

1. **[!UICONTROL 완료]**&#x200B;를 클릭합니다. 제품 구성 그룹(예: **[!UICONTROL 영업 그룹]**)이 Brand Portal에 추가되었습니다.
...
>[!NOTE]
>
>직접 또는 Brand Portal에서 [[!UICONTROL Admin Console]](https://adminconsole.adobe.com/enterprise/overview)에 로그인할 수 있습니다. Admin Console에 직접 로그인하는 경우 아래 절차의 4-7단계에 따라 제품 프로필에 사용자를 추가합니다.
...
1. 제품 프로필에 대량 사용자를 추가하려면 다음 작업을 수행하십시오.
   ...
   * **[!UICONTROL 업로드]**&#x200B;를 클릭합니다.
   ...
   기본 제품 프로필, 즉 Brand Portal에 사용자를 추가한 경우 추가한 사용자의 이메일 ID로 시작 이메일이 전송됩니다. 초대된 사용자는 시작 이메일의 링크를 클릭하고 [!UICONTROL Adobe ID]을(를) 사용하여 로그인함으로써 Brand Portal에 액세스할 수 있습니다. 자세한 내용은 [처음 로그인 경험](../using/brand-portal-onboarding.md)을 참조하세요.

   사용자 지정 또는 새 제품 프로필에 추가된 사용자는 이메일 알림을 받지 못합니다.

## 사용자에게 관리자 권한 제공 {#provide-administrator-privileges-to-users}

Brand Portal 사용자에게 시스템 관리자 또는 제품 관리자 권한을 제공할 수 있습니다. [!UICONTROL Admin Console]에서 사용할 수 있는 제품 프로필 관리자, 사용자 그룹 관리자 및 지원 관리자 등의 다른 관리 권한을 제공하지 마십시오. 이러한 역할에 대한 자세한 내용은 [관리자 역할](https://helpx.adobe.com/enterprise/using/admin-roles.html)을 참조하세요.

>[!NOTE]
>
>직접 또는 Brand Portal에서 [[!UICONTROL Admin Console]](https://adminconsole.adobe.com/enterprise/overview)에 로그인할 수 있습니다. [!UICONTROL Admin Console]에 직접 로그인하는 경우 아래 절차의 4-8단계에 따라 사용자를 제품 프로필에 추가하십시오.
...
1. [!UICONTROL 사용자] 페이지에서 기본적으로 왼쪽 레일의 [!UICONTROL 사용자]가 선택됩니다. 관리자 권한을 제공하려는 사용자의 사용자 이름을 누릅니다.
...
1. **[!UICONTROL 역할]** 드롭다운 목록에서 적절한 역할을 선택합니다.
   ...
   그룹 구성원이 포털 또는 공유 링크에서 다운로드하는 이미지 파일(.jpeg, .tiff, .png, .bmp, .gif, .pjpeg, x-portable-anymap, x-portable-bitmap, x-portable-graymap, x-portable-pixmap, x-rgb, x-xbitmap, x-xpixmap, x-icon, image/photoshop, image/x-photoshop, .psd, image/vnd.adobe.photoshop)의 원본 표현물에 액세스할 수 있도록 하려면 해당 그룹에 대해 선택한 **[!UICONTROL 원본 액세스]** 옵션을 유지합니다. 기본적으로 모든 사용자에 대해 **[!UICONTROL 원본에 액세스]** 옵션이 선택됩니다. 사용자 그룹이 원본 렌디션에 액세스하지 못하도록 하려면 해당 그룹에 해당하는 옵션의 선택을 해제합니다.
