...
기본적으로 **[!UICONTROL 빠른 다운로드]** 구성이 활성화되어 있으므로 브랜드 포털에서 원하는 에셋 파일을 다운로드하는 데 걸리는 시간이 크게 줄어듭니다.
...
* **[!UICONTROL 도구]** > **[!UICONTROL 다운로드]**&#x200B;로 이동하고 **[!UICONTROL 빠른 다운로드]** 구성이 **[!UICONTROL 다운로드 설정]**&#x200B;에서 활성화되었는지 확인합니다.
...
* [관리자 권한](https://www.ibm.com/support/knowledgecenter/SSXMX3_3.9.9/kc/connect_welcome.html) 을 사용하여 브라우저의 익스텐션에 IBM Aspera Connect 3.9.9를 설치합니다.
...
## 파일 가속기 {#expected-download-performance-using-file-accelerator}을(를) 사용한 다운로드 성능 샘플
...
|---------------------------|-----------------------------------|---------------------------------------------|-------------------------------------------------------------------------|
| 미국 서부(캘리포니아 주) | 18년 | 36 | 57년 |
| 미국 서부(오레곤) | 42 | 36 | 57년 |
| 미국 동부(버지니아 주) | 85 | 35 | 58년 |
| APAC(도쿄) | 124년 | 36 | 57년 |
...
1. **Aspera Connect 클라이언트 다운로드**
   ...
   시스템에 Aspera Connect 클라이언트가 설치되어 있지 않거나 기존 Aspera Connect 클라이언트가 오래된 경우, 브라우저 페이지에 **[!UICONTROL 최신 버전 다운로드]**&#x200B;를 선택하여 시스템별 Aspera Connect 클라이언트를 다운로드할 수 있는 메시지가 표시됩니다.
...
1. 클라이언트가 성공적으로 설치되면 브라우저 페이지를 새로 고친 후 다운로드 단계를 다시 시작합니다.
   ...
   Aspera Connect를 처음 사용하는 경우 브라우저가 **[!UICONTROL IBM Aspera Connect]**&#x200B;를 사용하여 링크를 열라는 메시지를 표시합니다. 나중에 이 대화 상자를 건너뛰려면 **[!UICONTROL FASP 링크]**&#x200B;에 대한 내 선택 사항을 기억하십시오.
...
나중에 이 대화 상자를 건너뛰려면 **[!UICONTROL 이 호스트가 있는 모든 연결에 내 선택 사항을 사용하십시오]**.
다운로드가 시작됩니다. 다운로드 진행 상황을 대화 상자에 표시합니다. 대화 상자를 사용하여 **[!UICONTROL 일시 중지]**, **[!UICONTROL 다시 시작]** 또는 **[!UICONTROL 취소]**합니다.
Aspera Connect 응용 프로그램은 사용자가 모든 전송 세션을 보고 관리할 수 있는 시스템의 활동 창을 제공합니다. 자세한 내용은 [Aspera Connect 클라이언트 설명서](https://downloads.asperasoft.com/en/documentation/8)를 참조하십시오.
...
>[!NOTE]
>
>Aspera Connect 클라이언트 응용 프로그램에서는 ]**Preferences**[!UICONTROL &#x200B;의 탭 ]**Transfer**[!UICONTROL &#x200B;에서 다운로드한 파일을 저장할 위치&#x200B;**[!UICONTROL 를 항상 묻는 메시지가 표시되지 않는 알려진 제한 사항이 있습니다.]** 다운로드가 시작되기 전에 텍스트 상자 **[!UICONTROL 다운로드한 파일을]**&#x200B;에 저장합니다.
...
1. Campaign 컨트롤 패널(**[!UICONTROL Window 키 + X]**&#x200B;을 열고 **[!UICONTROL Campaign 컨트롤 패널]**)을 선택합니다.
...
IBM Aspera Connect 클라이언트 환경 설정에서 아이콘을 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 환경 설정]**&#x200B;을 선택하여 설정할 수 있는 유용한 기본 설정이 몇 가지 있습니다.
...
1. 사용자 컴퓨터에서 [https://test-connect.asperasoft.com](https://test-connect.asperasoft.com/)을(를) 방문하여 포트가 차단되어 있는지 확인하십시오.

   포트가 OK되지 않으면 네트워크 팀에 연락하여 포트 33001(TCP 및 UDP 모두)이 방화벽에서 차단되지 않았는지 확인합니다.

1. 포트가 정상 상태인 경우, [https://www.speedtest.net/](https://www.speedtest.net/)을 사용하여 사용 가능한 대역폭을 측정하여 네트워크 속도가 느려지지 않은지 확인하십시오.

   대역폭이 몇 가지(1-10Mbps) 또는 Kbps인 경우 Aspera 환경 설정을 사용하고 사용 가능한 대역폭과 동일한 대역폭을 제한하십시오.

1. Aspera 데모 서버의 다운로드 작동 여부를 확인하려면 [https://demo.asperasoft.com/aspera/user](https://demo.asperasoft.com/aspera/user)을 사용하십시오.\
   (로그인: 아스퍼웹, 암호: 데모스페르타 )
