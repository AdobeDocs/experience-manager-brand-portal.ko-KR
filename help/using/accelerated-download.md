...
* **IBM Aspera Connect 3.9.9 설치** 관리자 권한( )을 사용하여 브라우저의 확장에서`https://www.ibm.com/docs/en/aspera-connect/3.9.9`).
* Aspera 전송 클라이언트의 플랫폼 지원은 다음을 참조하십시오. [IBM Aspera Connect 플랫폼 지원 매트릭스](https://www.asperasoft.com/company/support/transfer-clients/).
...
| 클라이언트 위치 | 클라이언트와 서버 간 지연 시간(밀리초) | Aspera Connect File Transfer Accelerator를 사용한 속도(MBps) | Aspera File Transfer Accelerator로 2GB 파일을 다운로드하는 데 걸린 시간(초) |
|---------------------------|-----------------------------------|---------------------------------------------|-------------------------------------------------------------------------|
...
다운로드 가속화가 작동하지 않는 경우 다음 단계에 따라 문제를 해결하십시오.

1. 다음을 방문하여 포트가 차단되지 않았는지 확인 [https://test-connect.asperasoft.com](https://test-connect.asperasoft.com/) 사용자 컴퓨터에서.
