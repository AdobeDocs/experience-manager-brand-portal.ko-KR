...
# 기여 폴더를 Experience Manager Assets에 게시 {#using-asset-souring-in-bp}

적절한 권한이 있는 Brand Portal 사용자는 여러 에셋 또는 여러 에셋이 포함된 폴더를 기여도 폴더에 업로드할 수 있습니다. 하지만 Brand Portal 사용자는 에셋만 업로드할 수 있습니다. **새로 만들기** 폴더를 삭제합니다. 다음 **공유됨** 폴더는 기여를 위해 새 에셋을 만드는 동안 Brand Portal 사용자가 사용할 수 있는 기본 에셋(참조 콘텐츠) 배포를 위한 것입니다.
...
* [기여 폴더를 Experience Manager Assets에 게시](#publish-contribution-folder-to-aem)

## 에셋 요구 사항 다운로드 {#download-asset-requirements}

Brand Portal 사용자는 기여도 폴더를 Experience Manager Assets 사용자가 공유할 때마다 이메일/펄스 알림을 자동으로 수신하므로 에서 간단한(에셋 요구 사항) 문서를 다운로드하고 기준 에셋(참조 콘텐츠)을 다운로드할 수 있습니다. **공유됨** 에셋 요구 사항을 이해할 수 있도록 해줍니다.

Brand Portal 사용자는 다음 활동을 수행하여 에셋 요구 사항을 다운로드합니다.

* **개요 다운로드**: 기여 폴더에 첨부된 간단한 개요(자산 요구 사항 문서)를 다운로드합니다. 이 문서에는 자산 유형, 용도, 지원되는 형식, 최대 자산 크기 등의 자산 관련 정보가 포함되어 있습니다.
* **기준 자산 다운로드**: 필요한 에셋 유형을 이해하는 데 사용할 수 있는 기준 에셋을 다운로드합니다. Brand Portal 사용자는 이러한 에셋을 참조로 사용하여 기여를 위한 새 에셋을 만들 수 있습니다.
...
**에셋 요구 사항을 다운로드하려면 다음을 수행하십시오.**

1. Brand Portal 인스턴스에 로그인.
1. Brand Portal 대시보드에서 기여도 폴더를 선택합니다.
1. 클릭 **[!UICONTROL 속성]**. 기여도 폴더 세부 정보가 포함된 속성 창이 열립니다.

   ![](assets/properties.png)

   ![](assets/download-asset-requirement2.png)

1. 을(를) 클릭합니다 **[!UICONTROL 개요 다운로드]** 로컬 컴퓨터에서 자산 요구 사항 문서를 다운로드하는 옵션입니다.

   ![](assets/download.png)

1. Brand Portal 대시보드로 돌아갑니다.
1. 기여도 폴더를 열려면 클릭하면 두 개의 하위 폴더가 표시됩니다.**[!UICONTROL 공유됨]** 및 **[!UICONTROL 새로 만들기]** 기여도 폴더 내. 공유 폴더에는 관리자가 공유하는 모든 기준 에셋(참조 콘텐츠)이 포함됩니다.
1. 다음을 다운로드할 수 있습니다. **[!UICONTROL 공유됨]** 로컬 컴퓨터의 모든 기준 자산이 들어 있는 폴더입니다.
또는 **[!UICONTROL 공유됨]** 폴더를 클릭하고 **다운로드** 아이콘을 클릭하여 개별 파일/폴더를 다운로드합니다.
...
>[!NOTE]
>
>Brand Portal 사용자는 에셋을 업로드할 수 있습니다(최대 **2**&#x200B;파일 크기당 GB)를 새 폴더에 추가합니다.
>
>Brand Portal 테넌트의 최대 업로드 제한은 다음과 같습니다. **10**&#x200B;모든 기여도 폴더에 누적되어 적용되는 GB입니다.
>
...
>
>Brand Portal 테넌트의 업로드 제한을 확장해야 하는 경우 **10** GB입니다. 요구 사항을 명시하여 고객 지원 센터에 문의하십시오.
...
1. 기여도 폴더를 선택하고 을(를) 클릭하여 엽니다. 기여 폴더에는 두 개의 하위 폴더가 있습니다. **[!UICONTROL 공유됨]** 및 **[!UICONTROL 새로 만들기]**.
...
에셋 요구 사항을 완료했는지 확인하고 새로 만든 에셋을에 업로드하십시오. **새로 만들기** 기여도 폴더 내의 폴더입니다.
...
1. **대기열에 추가됨** - Brand Portal에서 게시 워크플로우가 트리거되면 Brand Portal 사용자 및 Brand Portal 관리자에게 알림이 전송됩니다.

1. **완료** - 기여 폴더가 Brand Portal에 성공적으로 게시되면 Experience Manager Assets 사용자 및 Brand Portal 관리자에게 알림이 전송됩니다.
...
* Brand Portal에서 다음으로 이동 **[!UICONTROL 도구]** > **[!UICONTROL 자산 기여 상태]**. 이 보고서는 게시 작업 과정의 여러 단계에 있는 모든 게시 작업의 상태를 반영합니다.

   ![](assets/contribution-folder-status-v2.png)

* Experience Manager Assets(온-프레미스 또는 관리 서비스)에서 다음 위치로 이동합니다. **[!UICONTROL 에셋]** > **[!UICONTROL 작업]**. 이 보고서는 모든 게시 작업의 최종 상태(성공 또는 오류)를 반영합니다.

   ![](assets/publishing-status.png)

* Experience Manager Assets as a Cloud Service에서 **[!UICONTROL 에셋]** > **[!UICONTROL 작업]**.

   또는 다음 위치로 직접 이동할 수 있습니다. **[!UICONTROL 작업]** 전역 탐색에서.
...
이제 Brand Portal은 12시간마다 자동 작업을 실행하여 모든 기여 폴더를 스캔하고 AEM에 게시된 모든 에셋을 삭제합니다. 따라서 폴더 크기를 폴더 아래에 유지하기 위해 기여도 폴더의 자산을 수동으로 삭제할 필요가 없습니다. [임계값 제한](#upload-new-assets-to-contribution-folder). 지난 7일 동안 자동으로 실행된 삭제 작업의 상태를 모니터링할 수도 있습니다. 작업 보고서는 다음 세부 정보를 제공합니다.
...
1. Brand Portal에서 다음으로 이동 **[!UICONTROL 도구]**>**[!UICONTROL 자산 기여 상태]**>**[!UICONTROL 삭제 보고서]** 옵션을 선택합니다.

1. 작업을 선택하고 **[!UICONTROL 보기]** 보고서를 봅니다.

   삭제 작업에 포함된 각 에셋의 세부 정보를 확인합니다. 자산 제목, 크기, 작성자, 삭제 상태 및 삭제 시간 등 세부 정보가 보고서에 포함됩니다. 클릭 **[!UICONTROL 다운로드]** 작업에 대한 보고서를 CSV 형식으로 다운로드합니다.

   보고서에서 에셋의 삭제 상태는 다음과 같은 값을 가질 수 있습니다.

   * **삭제됨** - 에셋이 기여 폴더에서 정상적으로 삭제되었습니다.

   * **찾을 수 없음** - Brand Portal이 기여 폴더에서 자산을 찾을 수 없습니다. 에셋이 이미 폴더에서 수동으로 삭제되었습니다.

   * **건너뜀** - 기여도 폴더의 에셋에 사용할 수 있는 새 버전이 아직 Experience Manager에 게시되지 않았기 때문에 Brand Portal에서 에셋 삭제를 건너뛰었습니다.

   * **실패** - Brand Portal이 자산을 삭제하지 못했습니다. 를 사용하여 에셋을 삭제하려고 세 번 다시 시도합니다. `Failed` 상태를 삭제합니다. 에셋이 세 번째 삭제 재시도에 실패하면 에셋을 수동으로 삭제해야 합니다.
...
1. 다음으로 이동 **[!UICONTROL 도구]**>**[!UICONTROL 자산 기여 상태]**>**[!UICONTROL 삭제 보고서]** 옵션을 선택합니다.
