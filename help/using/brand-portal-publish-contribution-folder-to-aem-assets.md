...
Brand Portal 사용자가 기여 폴더를 공유할 때 Experience Manager Assets 사용자는 자동으로 이메일 및 펄스 알림을 받습니다. 이 워크플로를 통해 사용자는 **SHARED** 폴더에서 간단한(자산 요구 사항) 문서 및 기준 자산(참조 컨텐츠)을 다운로드하여 자산 요구 사항을 이해할 수 있습니다.

Brand Portal 사용자는 다음 활동을 수행하여 에셋 요구 사항을 다운로드합니다.

* **개요 다운로드** - 기여 폴더에 첨부된 개요(자산 요구 사항 문서)를 다운로드합니다. 여기에는 에셋 유형, 목적, 지원되는 형식, 최대 에셋 크기 등과 같은 에셋 관련 정보가 포함되어 있습니다.
* **기본 에셋 다운로드** - 필요한 에셋 유형을 이해하는 데 사용할 수 있는 기본 에셋을 다운로드합니다. Brand Portal 사용자는 이러한 에셋을 참조로 사용하여 기여를 위한 새 에셋을 만들 수 있습니다.

Brand Portal 대시보드는 새로 공유된 기여 폴더와 함께 Brand Portal 사용자에게 허용된 모든 기존 폴더를 반영합니다. 이 예에서 Brand Portal 사용자는 새로 생성된 기여 폴더에만 액세스할 수 있습니다. 다른 기존 폴더는 사용자와 공유되지 않습니다.
...
1. 기여도 폴더를 클릭하여 엽니다. 기여도 폴더 내에 **[!UICONTROL 공유]** 및 **[!UICONTROL 새로 만들기]**&#x200B;와 같은 두 개의 하위 폴더가 표시됩니다. 공유 폴더에는 관리자가 공유하는 모든 기준 에셋(참조 콘텐츠)이 포함됩니다.
1. 로컬 컴퓨터의 모든 기본 자산이 포함된 **[!UICONTROL SHARED]** 폴더를 다운로드할 수 있습니다.
또는 **[!UICONTROL SHARED]** 폴더를 열고 **Download** 아이콘을 클릭하여 개별 파일/폴더를 다운로드할 수 있습니다.

   ![](assets/download.png)

   ![](assets/download-asset-requirement5.png)

개요(자산 요구 사항 문서)를 살펴보고 기준 자산을 참조하여 자산 요구 사항을 이해합니다. 이제 기여에 대한 새 에셋을 만들고 기여도 폴더에 업로드할 수 있습니다.
...
>[!NOTE]
>
>Brand Portal 사용자는 에셋(파일 크기당 최대 2GB)을 NEW 폴더에 업로드할 수 있습니다.
>
>모든 Brand Portal 테넌트에 대한 최대 업로드 제한은 10GB이며, 모든 기여 폴더에 누적되어 적용됩니다.
>
>Brand Portal에 업로드된 자산은 렌디션에 대해 처리되지 않으며 미리 보기를 포함하지 않습니다.
...
Brand Portal 사용자는 새로 만든 에셋을 Experience Manager Assets에 게시한 후 NEW 폴더에서 삭제할 수 있습니다. 그러나 Brand Portal 관리자는 NEW 및 SHARED 폴더에서 자산을 삭제할 수 있습니다.

기여도 폴더를 만든 목표가 달성되면 Brand Portal 관리자는 기여도 폴더를 삭제하여 다른 사용자의 업로드 공간을 해제할 수 있습니다.

## 게시 작업 상태 {#publishing-job-status}

관리자는 두 개의 보고서를 사용하여 Brand Portal에서 Experience Manager Assets으로 게시된 에셋 기여 폴더의 상태를 볼 수 있습니다.
...
또한 Brand Portal을 사용하면 하나 이상의 보고서를 선택하고 수동으로 삭제할 수 있습니다.
