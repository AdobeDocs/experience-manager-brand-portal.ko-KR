...
>[!NOTE]
>
>비디오를 사용하여 작업하고 Brand Portal에 게시하려면 Experience Manager 작성자 인스턴스가 Dynamic Media 하이브리드 모드 또는 Dynamic Media **[!DNL Scene7]** 모드로 설정되어 있는지 확인하십시오.
...
>[!NOTE]
>
>**[!UICONTROL Scene7 연결]** 실행 모드에서 실행되는 Experience Manager 작성자로 구성된 Brand Portal 테넌트에서는 동적 비디오가 지원되지 않습니다.

## 다이내믹 비디오는 어떻게 재생됩니까? {#how-are-dynamic-videos-played}

![클라우드에서 비디오 인코딩을 가져옵니다](assets/VideoEncodes.png)

Brand Portal에 Dynamic Media 구성([Hybrid](../using/dynamic-video-brand-portal.md#configure-dm-hybrid-settings) 또는 [[!DNL Scene7]](../using/dynamic-video-brand-portal.md#configure-dm-scene7-settings) 구성)이 설정된 경우 **[!DNL Scene7]** 서버에서 동적 변환을 가져옵니다. 따라서 비디오 인코딩은 품질에서 지연 및 왜곡 없이 미리 보고 재생됩니다.

Brand Portal 저장소는 비디오 인코딩을 저장하지 않고 **[!DNL Scene7]** 서버에서 가져옵니다. Adobe Experience Manager 작성자 인스턴스와 Brand Portal의 Dynamic Media 구성이 동일한지 확인합니다.
...
* **Experience Manager 작성자 인스턴스에서 Dynamic Media Cloud Services 구성**
  ...
  Experience Manager 작성자가 실행 중인 Dynamic Media 모드(Scene7 모드 또는 하이브리드 모드)를 기반으로 [도구[!DNL Scene7]에서 Experience Manager 작성자에 대해 &#x200B;](https://experienceleague.adobe.com/ko/docs/experience-manager-65/content/assets/dynamic/config-dms7#configuring-dynamic-media-cloud-services)Dynamic Media 클라우드 서비스([&#x200B; 모드)](https://experienceleague.adobe.com/ko/docs/experience-manager-65/content/assets/dynamic/config-dms7#configuring-dynamic-media-cloud-services) 또는 **Dynamic Media 클라우드 서비스(하이브리드 모드)**&#x200B;를 설정하십시오. | **클라우드 서비스** | **Dynamic Media**.

* **Brand Portal에서 Dynamic Media 구성**

  Experience Manager 작성자의 Dynamic Media 클라우드 구성을 기반으로 Brand Portal 관리 도구에서 [Dynamic Media 설정](#configure-dm-hybrid-settings) 또는 [[!DNL Scene7] 설정](#configure-dm-scene7-settings)을(를) 구성하십시오.

  [별도의 Brand Portal 테넌트](#separate-tenants)가 Dynamic Media - **[!UICONTROL Scene7]** 모드 및 Dynamic Media - 하이브리드 모드에서 구성된 Experience Manager 작성자 인스턴스에 사용되는지 확인하십시오. Dynamic Media **[!UICONTROL S7]** 및 Dynamic Media Hybrid의 기능을 사용하는 경우 이 방법은 특히 중요합니다.
...
* SPS에서 보안 IP 미리 보기가 활성화된 경우 **허용 목록에 추가하다**
  ...
  Dynamic Media-**[!DNL Scene7]**(회사에 대해 [보안 미리 보기가 활성화됨](https://experienceleague.adobe.com/ko/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public))을(를) 사용하는 경우, SPS(**[!DNL Scene7]** Scene7[&#x200B; Publishing System) 플래시 UI를 사용하는 각 지역에 대해 &lbrace;3 회사 관리자 &#x200B;](https://experienceleague.adobe.com/ko/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public#testing-the-secure-testing-service)공개 이그레스 IP를 허용 목록&#x200B;**[!UICONTROL 하는 것이 좋습니다.]**
  ...
  허용 목록에 추가하다 이러한 이그레스 IP 중 0개를 테스트하려면 [보안 테스트 서비스를 위한 계정 준비&lbrace;를 참조하십시오.](https://experienceleague.adobe.com/ko/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public#testing-the-secure-testing-service)

## 모범 사례

Brand Portal(및 공유 링크)에서 다이내믹 비디오 자산을 성공적으로 미리 보고, 재생하고, 다운로드하도록 하려면 다음 지침을 따르십시오.
...
* **[!UICONTROL 지역]**
* Dynamic Media - **[!UICONTROL 모드의]**&#x200B;회사&#x200B;**[!DNL Scene7]**

### 허용 목록에 추가하다 Dynamic Media Scene7 모드에 대한 공개 이그레스

**[!UICONTROL 보안 미리 보기가 활성화됨]**&#x200B;인 Dynamic Media [Scene7](https://experienceleague.adobe.com/ko/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public)이(가) Brand Portal에 비디오 자산을 제공하는 데 사용되는 경우 **[!UICONTROL Scene7]**&#x200B;은(는) 스테이징 환경 또는 내부 응용 프로그램을 위한 전용 이미지 서버를 설정합니다. 이 서버에 대한 모든 요청은 원본 IP 주소를 확인합니다. 수신 요청이 승인된 IP 주소 목록 내에 없는 경우 실패 응답이 반환됩니다.
...
허용 목록에 추가하다 이러한 이그레스 IP 중 0개를 테스트하려면 [&#128279;](https://experienceleague.adobe.com/ko/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public#testing-the-secure-testing-service)보안 테스트 서비스를 위한 계정 준비&lbrace;를 참조하십시오.
...
1. **[!UICONTROL 등록 ID]** 및 **[!UICONTROL 비디오 서비스 URL]**(DM 게이트웨이 URL)을 지정하십시오. 이러한 세부 정보가 Experience Manager 작성자 인스턴스의 **[!UICONTROL 도구 > 클라우드 서비스]**&#x200B;에 있는 세부 정보와 같은지 확인하십시오.
...
Brand Portal 테넌트에서 Dynamic Media **[!UICONTROL Scene7]** 구성을 설정하려면 다음을 수행하십시오.
...
2. 관리 도구 패널에서 **[!UICONTROL Dynamic Media 구성]** 타일을 선택합니다.
   ...
   **[!UICONTROL Dynamic Media 구성 편집]** 페이지가 나타납니다.
...
6. 변경 내용을 지우고 암호를 재설정하고 구성을 기본 상태로 복원하려면 **[!UICONTROL 재설정]**&#x200B;을 선택하십시오.
