...
description: 모든 사용자는 동시에 액세스 가능한 자산과 폴더를 다운로드할 수 있습니다. 이 방법으로 승인된 브랜드 자산은 오프라인에서 사용할 수 있도록 안전하게 분배할 수 있습니다.
...
# 에셋 다운로드 {#download-assets-from-bp}
...
Adobe Experience Manager Assets Brand Portal은 사용자가 Brand Portal에서 액세스할 수 있는 자산 및 폴더를 동시에 다운로드할 수 있도록 함으로써 다운로드 환경을 개선합니다. 이 방법으로 승인된 브랜드 자산은 오프라인에서 사용할 수 있도록 안전하게 분배할 수 있습니다. Brand Portal에서 자산(승인된 자산)을 다운로드하는 방법과 [성능 다운로드](../using/brand-portal-download-assets.md#expected-download-performance).


>[!NOTE]
>
>Brand Portal 2020.10.0(및 이상)에서 **[!UICONTROL 빠른 다운로드]** 이 설정은 기본적으로 IBM Aspera Connect를 사용하여 자산의 빠른 다운로드를 지원합니다. IBM Aspera Connect 3.9.9 설치(`https://www.ibm.com/docs/en/aspera-connect/3.9.9`)을 클릭하여 Brand Portal에서 자산을 다운로드하십시오. 자세한 내용은 [Brand Portal에서 다운로드를 가속화하는 안내서](../using/accelerated-download.md).
>
>IBM Aspera Connect를 사용하지 않고 일반 다운로드 프로세스를 계속 수행하려면 Brand Portal 관리자에게 문의하여 **[!UICONTROL 빠른 다운로드]** 설정
...
>[!NOTE]
>
>사용자 인터페이스에 적용된 다운로드 설정을 통해 Brand Portal 사용자가 자산 표현물을 쉽게 구성하고 다운로드할 수 있는 셀프서비스 경험을 용이하게 할 수 있습니다. 애플리케이션 계층에서 자산 다운로드를 제한하지 않습니다. 예를 들어 사용자는 여전히 전체 URL 경로를 사용하여 자산 표현물에 액세스하고 다운로드할 수 있습니다.
...
관리자는 자산을 활성화할 수 있습니다 **[!UICONTROL 다운로드 설정]** 를 클릭하여 Brand Portal 사용자가 다운로드할 수 있는 렌디션 세트를 정의합니다.

사용 가능한 설정은 다음과 같습니다.

* **[!UICONTROL 빠른 다운로드]**

   IBM Aspera Connect를 사용하여 자산의 빠른 다운로드를 제공합니다. 기본적으로 **[!UICONTROL 빠른 다운로드]** 이 설정은에서 활성화됩니다. **[!UICONTROL 다운로드 설정]**.

* **[!UICONTROL 사용자 지정 표현물]**

   자산의 사용자 지정 및 (또는) 동적 표현물을 다운로드할 수 있습니다.

   원래 자산 및 시스템에서 생성한 표현물 이외의 모든 자산 표현물을 사용자 지정 표현물이라고 합니다. 여기에는 자산에 사용할 수 있는 동적 표현물 및 정적 표현물이 포함됩니다. 모든 사용자는 Experience Manager Assets에서 사용자 지정 정적 렌디션을 만들 수 있지만 관리자만 사용자 지정 동적 렌디션을 만들 수 있습니다. 자세한 내용은 [이미지 사전 설정 또는 동적 변환을 적용하는 방법](../using/brand-portal-image-presets.md).
...
관리자로 Brand Portal 테넌트에 로그인하고 **[!UICONTROL 도구]** > **[!UICONTROL 다운로드]**.

관리자는 Brand Portal 사용자가 자산 표현물에 액세스하고 다운로드할 수 있도록 모든 설정 조합을 활성화할 수 있습니다.
...
>[!NOTE]
>
>관리자만 만료된 자산을 다운로드할 수 있습니다. 만료된 자산에 대한 자세한 내용은 [자산의 디지털 권한 관리](../using/manage-digital-rights-of-assets.md).

### 사용자 그룹 설정 구성 {#configure-user-group-settings}

추가 **[!UICONTROL 다운로드 설정]** Brand Portal 관리자는 원본 자산과 해당 표현물을 보고 다운로드할 다양한 사용자 그룹에 대한 설정을 추가로 구성할 수 있습니다.

관리자로 Brand Portal 테넌트에 로그인하고 **[!UICONTROL 도구]** > **[!UICONTROL 사용자]**. 에서 **[!UICONTROL 사용자 역할]** 페이지로 이동하여 **[!UICONTROL 그룹]** 탭을 클릭하여 사용자 그룹의 보기 및 (또는) 다운로드 설정을 구성합니다.
...
다음 매트릭스는 사용자가 변환에 액세스할 수 있는지 여부를 [구성 다운로드](#configure-download):
...
>[!NOTE]
>
>다음 **[!UICONTROL 다운로드]** 대화 상자는 **[!UICONTROL 사용자 지정 표현물]** 및 (또는) **[!UICONTROL 시스템 표현물]** 에서 활성화됨 **[!UICONTROL 다운로드 설정]**.
...
1. Brand Portal 테넌트에 로그인합니다. 기본적으로 **[!UICONTROL 파일]** 게시된 모든 자산 및 폴더가 포함된 보기가 열립니다.

   다음 중 하나를 수행하십시오.

   * 다운로드할 자산 또는 폴더를 선택합니다. 맨 위에 있는 도구 모음에서 **[!UICONTROL 다운로드]** 아이콘.

      ![다중 자산 선택](assets/select-assets-new.png)

   * 자산의 특정 자산 표현물을 다운로드하려면 포인터를 자산 위로 가져간 다음 **[!UICONTROL 다운로드]** 빠른 작업 미리 보기에서 사용할 수 있는 아이콘입니다.
      ...
      >[!NOTE]
      >
      >처음 자산을 다운로드하는 동안 브라우저에 IBM Aspera Connect가 설치되어 있지 않으면 Aspera 다운로드 가속기( )를 설치하라는 메시지가 표시됩니다`https://www.ibm.com/docs/en/aspera-connect/3.9.9`).


      >[!NOTE]
      >
      >다운로드하는 자산에 라이선스가 있는 자산도 포함되어 있으면 로 리디렉션됩니다 **[!UICONTROL 저작권 관리]** 페이지. 이 페이지에서 자산을 선택하고 **[!UICONTROL 동의]**&#x200B;를 클릭한 다음 **[!UICONTROL 다운로드]**. 동의하지 않기로 선택하면 라이선스가 있는 자산이 다운로드되지 않습니다.
      > 
      >라이선스로 보호된 자산에 [사용권 계약 첨부](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/drm.html) 이렇게 하려면 자산의 [메타데이터 속성](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/drm.html) Experience Manager Assets.


      ![라이선스 자산](assets/licensed-asset-new.png)

1. 다음 **[!UICONTROL 다운로드]** 선택한 모든 자산을 나열하는 대화 상자가 열립니다.

   자산을 클릭하여 사용 가능한 변환을 보고 다운로드할 표현물에 해당하는 확인란을 선택합니다.

   개별 자산에 대한 표현물을 수동으로 선택하거나 제외하거나 **적용** 유사한 자산 유형(이 예제의 모든 이미지 파일)에 대해 다운로드할 동일한 표현물 세트를 선택하는 아이콘입니다. 에서 **[!UICONTROL 모두 적용]** 대화 상자 **[!UICONTROL 완료]** 규칙을 유사한 모든 자산에 적용하려면

   ![모두 적용](assets/apply.png)

   필요한 경우 를 클릭하여 다운로드 목록에서 자산을 제거할 수도 있습니다 **제거** 아이콘.

   ![제거](assets/remove.png)

   자산을 다운로드하는 동안 Brand Portal 폴더 계층 구조를 유지하려면 **[!UICONTROL 각 자산에 대해 별도의 폴더를 만듭니다]** 확인란을 선택합니다.

   다운로드 단추는 선택한 항목의 수를 반영합니다. 규칙 적용을 완료하면 **[!UICONTROL 항목 다운로드]**.
...
>[!NOTE]
>
>만약 **[!UICONTROL 빠른 다운로드]** 관리자가 설정을 해제하면 선택한 표현물이 IBM Aspera Connect를 사용하지 않고 zip 폴더에서 직접 다운로드됩니다.

>[!NOTE]
>
>만약 **[!UICONTROL 자산 다운로드]** 이 설정은에서 활성화됩니다. **[!UICONTROL 다운로드 설정]**&#x200B;로 지정하는 경우, 자산 표현물은 zip 폴더 내의 각 자산에 대한 별도의 폴더로 다운로드됩니다.
>  
>자산이 공유 링크에서 다운로드되면 자산 표현물이 zip 폴더 내의 각 자산에 대한 별도의 폴더에 다운로드됩니다.
>
>폴더, 컬렉션 또는 20개 이상의 자산을 다운로드하도록 선택한 경우, **[!UICONTROL 다운로드]** 대화 상자를 건너뛰고 다이내믹 렌디션을 제외한 사용자가 액세스할 수 있는 모든 자산 렌디션은 zip 폴더에 다운로드됩니다.

>[!NOTE]
>
>Brand Portal은 - 하이브리드 및 Scene 7 모드에서 모두 Dynamic Media 구성을 지원합니다.
>
>(*Experience Manager Assets 작성자 인스턴스에서 실행 중인 경우&#x200B;**Dynamic Media 하이브리드 모드***)
>
...
그렇지 않으면 [관리자가 원래 표현물에 대한 액세스 권한을 갖도록 승인됨](../using/brand-portal-adding-users.md#main-pars-procedure-202029708)로 지정하는 경우 선택한 자산의 원래 표현물은 다운로드되지 않습니다.
...
사용자는 다양한 자산 표현물을 미리 보고, 특정 표현물을 선택하고, 에서 직접 자산 표현물을 다운로드할 수 있습니다 **[!UICONTROL 표현물]** 패널을 열 필요 없이 자산 세부 사항 페이지의 패널 **[!UICONTROL 다운로드]** 대화 상자.
...
1. 다음 **[!UICONTROL 표현물]** 패널에 자산을 기반으로 액세스할 수 있는 모든 자산 표현물이 나열됩니다 [구성 다운로드](#configure-download).

   다운로드할 특정 표현물을 선택하고 를 클릭합니다 **[!UICONTROL 항목 다운로드]**.
