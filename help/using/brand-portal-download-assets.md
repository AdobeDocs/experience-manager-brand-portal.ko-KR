...
1. 선택한 모든 자산이 **[!UICONTROL 나열된 다운로드]** 대화 상자가 열립니다.
   ...
   개별 자산에 대한 변환을 수동으로 선택하거나 제외하거나, **적용** 아이콘을 클릭하여 유사한 자산 유형(이 예에서는 모든 이미지 파일)에 대해 다운로드할 동일한 변환 세트를 선택할 수 있습니다. 모두 **[!UICONTROL 적용]** 대화 상자에서 **[!UICONTROL 완료를]** 클릭하여 모든 유사한 자산에 규칙을 적용합니다.
